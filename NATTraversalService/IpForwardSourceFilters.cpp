...
CIpForwardSourceFilters::CIpForwardSourceFilters(
...
CIpForwardSourceFilters::~CIpForwardSourceFilters(void)
{
...
void CIpForwardSourceFilters::CreateFilters(
    int nSingleIP,
    int nRangeIP,
    int nSubnetIP,
    ...
    )
{
    ...
    int     j = 0, i, nRangeIndex = 0, nSubnetIndex = 0;
    ...
    m_pOwner->m_nIPForwardSourceFilterCount = nFilterCount;
    m_pOwner->m_pIPForwardSourceFilterArray = pFilterIDsArray;
    RtlZeroMemory(
        pFilterIDsArray,
    ...
    m_pConditionsArray = new _dist_conditions[nFilterCount];
    RtlZeroMemory(
        m_pConditionsArray,
        sizeof(_dist_conditions)*nFilterCount
        );
    for(i=0;i<nSingleIP;i++)
    {
        ...
        m_pFiltersArray[j].displayData.name = L"NAT Travelsal IPFORWARD SOURCE Filter";
        ...
        m_pFiltersArray[j].numFilterConditions = 2;
        m_pFiltersArray[j].filterCondition = &m_pConditionsArray[j]._conditions[0];
        m_pFiltersArray[j].action.type = FWP_ACTION_PERMIT;
        m_pFiltersArray[j].action.calloutKey = GUID_NULL;
        m_pConditionsArray[j]._conditions[0].fieldKey = FWPM_CONDITION_DESTINATION_INTERFACE_INDEX;
        m_pConditionsArray[j]._conditions[0].matchType = FWP_MATCH_EQUAL;
        m_pConditionsArray[j]._conditions[0].conditionValue.type = FWP_UINT32;
        m_pConditionsArray[j]._conditions[0].conditionValue.uint32 = ulRouterIndex;
        m_pConditionsArray[j]._conditions[1].fieldKey = FWPM_CONDITION_IP_SOURCE_ADDRESS;
        m_pConditionsArray[j]._conditions[1].matchType = FWP_MATCH_EQUAL;
        m_pConditionsArray[j]._conditions[1].conditionValue.type = FWP_UINT32;
        m_pConditionsArray[j]._conditions[1].conditionValue.uint32 = nhIPAddress;
    ...
    for(i=0;i<nRangeIP;i++)
    {
        ...
        m_pConditionsArray[j]._conditions[0].conditionValue.uint32 = ulRouterIndex;
        m_pConditionsArray[j]._conditions[1].fieldKey = FWPM_CONDITION_IP_SOURCE_ADDRESS;
        m_pConditionsArray[j]._conditions[1].matchType = FWP_MATCH_RANGE;
        m_pConditionsArray[j]._conditions[1].conditionValue.type = FWP_RANGE_TYPE;
        m_pConditionsArray[j]._conditions[1].conditionValue.rangeValue = &m_pIp4RangeArray[nRangeIndex];
    ...
    for(i=0;i<nSubnetIP;i++)
    {
        ...
        m_pConditionsArray[j]._conditions[1].fieldKey = FWPM_CONDITION_IP_SOURCE_ADDRESS;
        m_pConditionsArray[j]._conditions[1].matchType = FWP_MATCH_EQUAL;
        m_pConditionsArray[j]._conditions[1].conditionValue.type = FWP_V4_ADDR_MASK;
        m_pConditionsArray[j]._conditions[1].conditionValue.v4AddrMask = &m_pV4AddMaskArray[nSubnetIndex];
        dwResult = FwpmFilterAdd0(
            m_pOwner->m_hEngineHandle,
            &m_pFiltersArray[j],
            NULL,
